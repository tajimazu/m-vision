...
# 1. ご飯リスト（梅干しご飯を追加！）
rice_list = [
    "白ご飯", "わかめご飯", "炊き込みご飯", "鮭フレークご飯", "玄米ご飯", 
    "雑穀米", "ゆかりご飯", "炒飯", "塩昆布おにぎり", "ツナマヨご飯", 
    "高菜ご飯", "菜飯", "カリカリ梅ご飯", "鰹節おかかご飯", "五目ちらし寿司", 
...
if uploaded_file is not None:
    ...
    """, language="text")
        ...
        for i in [4, 5]:
            key = f"s{i}"
            item = p[key]
            st.session_state.locks[key] = st.checkbox(f"🥢 おかず{i}: {item['name']}", value=st.session_state.locks["s5" if i==5 else "s4"])
...
st.caption("Bento Vision - Updated Edition")
