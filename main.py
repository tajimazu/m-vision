...
# 提案メニュー
menu_options = [
    {"name": "定番！彩り三色わっぱ弁当", "red": "ミニトマト", "green": "ブロッコリー", "yellow": "卵焼き", "tip": "定番の赤・緑・黄で、まずは基本を抑えましょう！"},
...
if uploaded_file is not None:
    ...
    if st.session_state.proposal is None:
        ...
        <div style="background-color: white; padding: 25px; border-radius: 15px; border-left: 10px solid #ff9e1b; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">
            ...
            <p style="font-size: 18px; margin: 10px 0;">🔴 <b>赤：{p['red']}</b></p>
            <p style="font-size: 18px; margin: 10px 0;">🟢 <b>緑：{p['green']}</b></p>
            <p style="font-size: 18px; margin: 10px 0;">🟡 <b>黄：{p['yellow']}</b></p>
            <div style="background-color: #fff9f0; padding: 15px; border-radius: 10px; margin-top: 15px;">
